...
> **Unity Editor task (user):** Open `Assets/Scenes/SampleScene.unity`. Select the GameObject that holds `BattleController`. Add a `SpellCastController` component to the same GameObject via Add Component → Scripts → Axiom.Voice → Spell Cast Controller.
...
**Task 6 check-in:** `SampleScene.unity`. ✓  
