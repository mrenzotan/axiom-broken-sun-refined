...
1. **Parent agent** loads standards once (`docs/GAME_PLAN.md`, `AGENTS.md`, scope from user) and defines non-overlapping slices.
...
| Subagent | Focus |
|----------|--------|
| Architecture | Layout vs `AGENTS.md`, forbidden singletons, SO vs hardcoded data, UI folder rules |
...
| `AGENTS.md` | Folder layout, singleton policy, voice threading, dead-code policy |
...
- `static` / `Singleton` / `Instance` — anything beyond allowed `GameManager` pattern (`AGENTS.md`)
...
- MonoBehaviour thinness: logic bloated in behaviours vs plain C# classes (`GAME_PLAN` / `AGENTS.md`)
...
- Edit Mode vs Play Mode placement vs `AGENTS.md` / existing `Assets/Tests/**` layout
