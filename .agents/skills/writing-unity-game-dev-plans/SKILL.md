...
**Codex writes all C# scripts directly. The user performs all Unity Editor tasks.**
...
> **CRITICAL:** This project uses **Unity Version Control (UVCS)**, NOT git. AGENTS.md may say "Git (local)" — ignore that for commit steps. UVCS is the source of truth for version control in this project.
...
| Using `git add` / `git commit` | **UVCS only.** AGENTS.md may mention "Git (local)" — that is stale. Every commit step must use Unity Version Control → Pending Changes → Check in. |
