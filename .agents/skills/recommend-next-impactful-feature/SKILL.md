...
1. If the repo has `AGENTS.md`, read it first for authoritative paths, current phase, architecture rules, and “not yet implemented” callouts.
2. If `AGENTS.md` disagrees with a design doc, **trust `AGENTS.md` for repo state** and note the doc drift.

## Source map (this project)

Adjust only if `AGENTS.md` or folder layout differs.
...
- Entry scenes, managers, and cross-scene state mentioned in `AGENTS.md`
...
- Respect documented **non-negotiables** (e.g. architecture constraints in `AGENTS.md`).
...
For chemistry combat, spells, or enemies, read the authoritative mechanics doc referenced from `AGENTS.md` before arguing for combat-adjacent work.
