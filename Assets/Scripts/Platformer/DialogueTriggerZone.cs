...
namespace Axiom.Platformer
{
    ...
    public class DialogueTriggerZone : MonoBehaviour
    {
        ...
        [SerializeField]
        [Tooltip("Optional UI prompt to show when player is in range (e.g., E key indicator).")]
        ...
        private void OnTriggerEnter2D(Collider2D other)
        {
            ...
            Debug.Log($"[DialogueTriggerZone] Interaction Prompt ref: {_interactionPrompt}");
            if (_interactionPrompt != null)
            {
                Debug.Log("[DialogueTriggerZone] Calling Show()");
                _interactionPrompt.Show();
            }
            else
            {
                Debug.LogError("[DialogueTriggerZone] _interactionPrompt is NULL!");
            }
