...
namespace Axiom.Battle.UI
{
    ...
    public class BattleTutorialPromptUI : MonoBehaviour
    {
        ...
        public void ShowAndPause(string body, Action onContinue)
        {
            ...
            // If a scene transition is still running (e.g. the white-flash fade-in after a battle
            // load), freezing now would set Time.timeScale = 0, which stalls the fade-in coroutine
            // (it advances on Time.deltaTime) — leaving the overlay opaque forever. Defer the show +
